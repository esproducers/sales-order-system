...
import { updateClientAdmin, getClientAdmin } from '@/actions/clients'
...
export default function EditClientPage() {
    ...
    const [existingSsmUrl, setExistingSsmUrl] = useState<string | null>(null)
    ...
    const loadClient = async () => {
        try {
            setLoading(true)
            const { data, error } = await getClientAdmin(clientId, profile!.id)
            ...
            if (data) {
                ...
                setExistingSsmUrl(data.ssm_file_url || null)
            }
        } catch (error: any) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setSaving(true)
        try {
            let ssmFileUrl = existingSsmUrl
            if (ssmFile) {
                const uploadedUrl = await uploadSSMFile()
                if (uploadedUrl) ssmFileUrl = uploadedUrl
            }
            ...
            const { error } = await updateClientAdmin(clientId, profile.id, clientData)
    ...
    if (loading || !profile) {
        return (
            <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <p>Loading client info... Please wait.</p>
            </div>
        )
    }
        ...
        <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
            ...
            <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 16px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
                    <div>
                        <h1 style={{ fontSize: '2rem', fontWeight: 800, margin: 0 }}>Edit Client</h1>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 20, alignItems: 'flex-start' }}>
                    ...
                    <div style={{ background: '#fff', borderRadius: 12, boxShadow: '0 1px 4px rgba(0,0,0,0.06)', overflow: 'hidden' }}>
                        ...
                        <form onSubmit={handleSubmit} style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 20 }}>
                            ...
                            {/* SSM Upload */}
                            <div>
                                <label style={labelStyle}>SSM Document</label>
                                {(existingSsmUrl && !ssmFile) && (
                                    <div style={{ marginBottom: 16, fontSize: '0.85rem' }}>
                                        <a href={existingSsmUrl} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--primary)', fontWeight: 600, textDecoration: 'none' }}>📎 View current SSM Document</a>
                                    </div>
                                )}
                                <label
                                    style={{
                                        ...
                                        {ssmFile ? (
                                            ...
                                            <>
                                                ...
                                                <div style={{ fontSize: '0.75rem', color: '#9ca3af', marginTop: 2 }}>
                                                    PDF, JPG, or PNG up to 5MB (Replaces old file)
                                                </div>
                                            </>
                                        )}
                                    </div>
                            ...
                            <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end', paddingTop: 8, borderTop: '1px solid #f0f0f0' }}>
                                ...
                                <button
                                    ...
                                    style={{
                                        padding: '10px 28px',
                                        background: (saving || uploadingFile) ? '#9ca3af' : 'var(--primary)',
                                        color: '#fff',
                                        border: 'none',
                                        borderRadius: 8,
                                        fontWeight: 700,
                                ...
                                >
                                    {saving ? 'Saving…' : uploadingFile ? 'Uploading…' : 'Save Changes'}
                                </button>
                            </div>
                        </form>
                    </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                        ...
                        >
                            ...
                            <p style={{ margin: 0, fontSize: '0.8rem', color: '#374151', lineHeight: 1.5 }}>
                                Keeping client records up-to-date helps provide smoother services and prevents issues during order fulfillment.
                            </p>
                        </div>
                    </div>
                </div>
