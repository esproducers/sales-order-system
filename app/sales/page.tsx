                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-col xl:flex-row items-start xl:items-center justify-between mb-6 md:mb-8 gap-5">
                        <div className="w-full xl:w-auto">
                            <h1 className="text-2xl md:text-3xl font-extrabold text-gray-900 m-0">My Sales</h1>
                            <p className="text-gray-500 mt-2 text-sm md:text-base">Full record of your sales performance</p>
                        </div>
                        <div className="flex flex-col sm:flex-row w-full xl:w-auto gap-4">
                            ...
                            <div className="flex-1 xl:w-[220px] bg-white rounded-xl p-5 md:p-6 text-center shadow-[0_1px_4px_rgba(0,0,0,0.06)] border border-gray-100">
                                ...
                                <div className="text-xl md:text-2xl font-black text-primary-dark truncate">RM {totalCommissionCounted.toFixed(2)}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-[0_1px_4px_rgba(0,0,0,0.06)] border border-gray-100 overflow-hidden text-sm">
                        {loading ? (
                            <div className="p-16 text-center text-gray-500 font-medium">Loading sales history…</div>
                        ) : filtered.length === 0 ? (
                            <div className="p-16 text-center text-gray-500 font-medium bg-gray-50 rounded-2xl mx-4 my-4 border-2 border-dashed border-gray-200">
                                No sales records found matching your current filters.
                            </div>
                        ) : (
                            <div className="flex flex-col">
                                ...
                                <div className="hidden md:grid grid-cols-4 bg-gray-50 border-b-2 border-gray-100 p-4">
                                     ...
                                     <div className="font-bold text-gray-500 uppercase tracking-widest text-xs">Order ID</div>
                                     <div className="font-bold text-gray-500 uppercase tracking-widest text-xs">Client</div>
                                     <div className="font-bold text-gray-500 uppercase tracking-widest text-xs text-right">Total Sales</div>
                                </div>
                                
                                {/* Rows (Stacked on Mobile, Grid on Desktop) */}
                                {filtered.map((s) => (
                                    <div key={s.id} className="flex flex-col md:grid md:grid-cols-4 border-b border-gray-100 p-4 md:p-5 gap-y-2 md:gap-y-0 text-gray-800 hover:bg-gray-50 transition-colors">
                                        ...
                                        <div className="flex justify-between items-center md:block">
                                            ...
                                            <span className="font-bold text-gray-900 md:font-semibold">{format(new Date(s.purchase_date), 'MMM d, yyyy')}</span>
                                        </div>

                                        {/* Mobile: Row 2, Desktop: Col 2 */}
                                        <div className="flex justify-between items-center md:block">
                                            <span className="md:hidden text-xs font-bold text-gray-400 uppercase tracking-widest">Order ID</span>
                                            <span className="font-mono text-gray-500 bg-gray-100 px-2 py-1 rounded text-xs md:text-sm md:bg-transparent md:p-0">#{s.id?.slice(0, 8).toUpperCase()}</span>
                                        </div>

                                        {/* Mobile: Row 3, Desktop: Col 3 */}
                                        <div className="flex justify-between items-center md:block">
                                            <span className="md:hidden text-xs font-bold text-gray-400 uppercase tracking-widest">Client</span>
                                            <span className="font-extrabold md:font-bold text-gray-700">{s.clients?.company_name}</span>
                                        </div>

                                        {/* Mobile: Row 4, Desktop: Col 4 */}
                                        <div className="flex justify-between items-center md:block md:text-right pt-2 mt-2 border-t border-dashed border-gray-200 md:border-none md:p-0 md:m-0">
                                            <span className="md:hidden text-xs font-bold text-gray-400 uppercase tracking-widest">Total Sales</span>
                                            <span className="font-black text-primary-dark text-lg md:text-base">RM {s.total_amount?.toFixed(2)}</span>
                                        </div>
                                ...
                                {/* Grand Total Footer */}
                                <div className="flex flex-col md:flex-row justify-between md:justify-end items-center px-5 py-6 bg-primary-light/50 border-t-2 border-gray-100 gap-3 md:gap-6">
                                     <div className="font-extrabold text-gray-800 text-sm md:text-base">
                                        GRAND TOTAL SALES:
                                     </div>
                                     <div className="font-black text-2xl md:text-3xl text-primary-dark">
                                        RM {totalSalesCounted.toFixed(2)}
                                     </div>
                                </div>
                            </div>
                        )}
